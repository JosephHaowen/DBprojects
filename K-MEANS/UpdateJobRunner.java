...
public class UpdateJobRunner
{
        ...
        String outputDirectory) throws Exception
    {
        ...
        for(int h = 0; h < KMeans.centroids.size(); h++){
            c_old.add(new Point(KMeans.centroids.get(h)));
        }  

        while(iter<=maxIterations&&isChanged){
	    update_c_old();
             
            try{
                thisJob[iter] = createUpdateJob(iter,inputDirectory,outputDirectory);
                thisJob[iter].waitForCompletion(true);
            }
            catch (Exception e){
                System.out.println("Failed to CreateUpdateJob.");
            }
            isChanged = detectChange();
            iter++;
        }
        return iter;
    }
    ...
    /**
    * Every time needs to update the c_old
    */
    public static void update_c_old(){
	for(int h = 0; h < KMeans.centroids.size(); h++){
                c_old.set(h,new Point(KMeans.centroids.get(h)));
        }  
    }

    /*
     *  To detect whether new centroids are changed, compare every point 
     */
    public static boolean detectChange(){
        for(int i = 0; i<c_old.size(); i++){
            if(KMeans.centroids.get(i).compareTo(c_old.get(i))!=0)
                return true;
        }
        return false;
    }
